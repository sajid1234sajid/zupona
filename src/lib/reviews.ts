...
function toReview(row: ReviewRow, images: string[] = []): Review {
  return {
    id: row.id,
...
export async function listReviews(
  ...
  const { results } = await db
    .prepare(
      `SELECT r.id, r.product_id, r.user_id, u.name AS user_name, r.order_item_id, r.rating,
  ...
  return results.map((row) =>
    toReview(
      row,
      imageRows.filter((image) => image.review_id === row.id).map((image) => image.url)
    )
  );
}
...
export async function createReview(input: ReviewInput): Promise<string> {
  ...
  const db = await getDB();
  const orderItemId = await findPurchase(input.userId, input.productId);
  const reviewId = crypto.randomUUID();

  const statements = [
    db
      .prepare(
        `INSERT INTO reviews (id, product_id, user_id, order_item_id, rating, title, body)
         VALUES (?, ?, ?, ?, ?, ?, ?)`
      )
      .bind(
        reviewId,
    ...
    ...(input.imageUrls ?? []).map((url) =>
      db
        .prepare("INSERT INTO review_images (id, review_id, url) VALUES (?, ?, ?)")
        .bind(crypto.randomUUID(), reviewId, url)
    ),
...
export async function setReviewApproval(
  ...
  const db = await getDB();
  await db.batch([
    db.prepare("UPDATE reviews SET is_approved = ? WHERE id = ?").bind(isApproved ? 1 : 0, reviewId),
    recalculateStatement(db, productId),
  ]);
}
